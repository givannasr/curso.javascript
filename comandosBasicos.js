//var numero1 = parseInt(prompt("Digite um número: "));
//var numero2 = parseInt(prompt("Digite o segundo número: "));
//var resultado = numero1+numero2;
//document.write("<h1>O resultado = </h1>"+resultado);

//var numB = parseFloat(prompt("Digite um número")); // num_b será do tipo number.
//alert(numB+" é do tipo "+typeof(numB));

//var valorDigitado = prompt("Digite:")
//var a = valorDigitado/3;
//document.write(a);
//a = a.toFixed(1);
//document.write("<br>"+a);//aredondamento

//var a = parseInt(prompt("Digite o valor"));
//var b = parseInt(prompt("Digite o valor"));
//var c = a%b;
//document.write("o módulo de "+a+" dividido por "+b+" é ="+c)

//var a = 5; 
//var b = 6;
//alert(a < b);

//var a = 3;
//var b = 5;
//alert(a <= b);

//var a = 4;
//var b = 6;
//alert(b > a);

//var a = 3;
//var b = 3;
//alert("A = B: "+(a==b))
//alert("A é diferente de B: "+(a!=b))

//var a = 6;
//var b = 3;
//var c = 10;
//alert((a>b)&&(b>c));

//var a = 6;
//var b = 3;
//var c = 10;
//alert((a>b)||(b>c));
//alert((a>b)&&(b>c));


//var filho = prompt("O seu filho está se formando? se sim digitar 1, se não digitar 0");
//var prova = prompt("Seu filho gabaritou a prova? Se sim 1, se não 0");
//alert("O filho vai ganhar um carro, se formar e gabaritar a prova");
//alert((filho==1)&&(prova==1));
